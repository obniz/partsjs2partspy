...
class ArduCAMMini:
    ...
    def spi_write(self, addr, byteData):
        data = []
        data.push(*[addr])
        data.push(*[byte_data])
    ...
    def i2c_byte_write(self, addr, byteData):
        self.i2c.write(*[self.sensor_addr, [addr, byte_data]])

    def i2c_regs_write(self, regs):
        for _i in regs:
            self.i2c.write(*[self.sensor_addr, _i])

    def spi_write_reg(self, addr, byteData):
    ...
    async def spi_pingpong_wait(self):
        testVal = 0x55
        self.spi_write_reg(*[self.regs._arduchip__test1, test_val])
        val = await self.spi_read_reg_wait(*[self.regs._arduchip__test1])
        if val != test_val:
            raise Exception('spi bus fail')

    def set_mode(self, mode):
        modes = {'_mcu2_lcd': 0x00, '_cam2_lcd': 0x01, '_lcd2_mcu': 0x02}
        if type(modes[mode]) != 'number':
            raise Exception('unknown mode. options are ' + modes)
        self.spi_write_reg(*[self.regs._arduchip__mode, modes[mode]])
    ...
    def init(self):
        ...
        self.obniz.wait(*[100])
        self.i2c_regs_write(*[self.configs._ov2640__jpeg__init])
        self.i2c_regs_write(*[self.configs._ov2640__yuv422])
        self.i2c_regs_write(*[self.configs._ov2640__jpeg])
        self.i2c_byte_write(*[0xff, 0x01])
    ...
    async def startup_wait(self):
        ...
        if chipid != 0x2642:
            raise Exception('unknown chip ' + chipid)
        self.init()
    ...
    def set_size(self, string):
        ...
        if map[string]:
            ...
            raise Exception('unsupported size options are ' + _object.keys(*[map]))

    def update_fifo(self, data):
        self.spi_write_reg(*[self.regs._arduchip__fifo, data])

    def flush_fifo(self):
        self.spi_write_reg(*[self.regs._arduchip__fifo, 0x01])

    async def read_fifolength_wait(self):
        len1 = await self.spi_read_reg_wait(*[self.regs._fifo__size1])
        len2 = await self.spi_read_reg_wait(*[self.regs._fifo__size2])
        len3 = await self.spi_read_reg_wait(*[self.regs._fifo__size3]) and 0x07
        return len3 << 16 or len2 << 8 or len1 and 0x07ffff

    def start_capture(self):
        self.spi_write_reg(*[self.regs._arduchip__fifo, 0x02])

    async def is_capture_done_wait(self):
        CAP_DONE_MASK = 0x08
        val = await self.spi_read_reg_wait(*[self.regs._arduchip__trig])
        return True if val and _cap__done__mask else False

    async def read_fifowait(self):
        length = await self.read_fifolength_wait()
        self.io_cs.output(*[False])
        self.spi.write(*[[self.regs._burst__fifo__read]])
        self.spi.write(*[[0xff]])
        buf = []
